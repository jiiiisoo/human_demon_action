...
# Import all utility functions from original finetune.py
...
def save_training_checkpoint(
    ...
    Args:
        ...
        train_dataset (LIBEROHdf5Dataset): Training dataset.
...
def finetune(cfg: FinetuneConfig) -> None:
    """
    Fine-tunes base VLA on LIBERO demonstration dataset via LoRA.

    Identical to original finetune() except uses LIBEROHdf5Dataset instead of RLDSDataset.
    ...
    if cfg.use_tracking_head:
        ...
        assert cfg.tracking_tracks_root is not None, "tracking_tracks_root is required when using the tracking head."
    ...
    dataloader = DataLoader(
        ...
        shuffle=True,  # Map-style dataset supports shuffling
        collate_fn=collator,
        num_workers=4,  # Can use multiple workers with map-style dataset
        pin_memory=True,
