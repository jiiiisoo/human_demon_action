...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _process_pointcloud_features(self, projected_patch_embeddings, pointcloud, pointcloud_projector):
        """Process pointcloud token and append to vision features"""
        if pointcloud_projector is not None and pointcloud is not None:
            # projected_patch_embeddings: (bsz, num_patches * num_images [+ proprio], llm_dim)
            # pointcloud: (bsz, num_points, point_dim) or (num_points, point_dim)
            if pointcloud.dim() == 2:
                pointcloud = pointcloud.unsqueeze(0)
            assert (
                pointcloud.shape[0] == projected_patch_embeddings.shape[0]
            ), "Pointcloud batch size must match projected patch embeddings."
            pointcloud_features = pointcloud_projector(pointcloud)  # (bsz, llm_dim)
            pointcloud_features = pointcloud_features.unsqueeze(dim=1)  # (bsz, 1, llm_dim)
            return torch.cat((projected_patch_embeddings, pointcloud_features), dim=1)
        return projected_patch_embeddings
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            # Add initial-frame pointcloud token if provided
            if pointcloud is not None:
                projected_patch_embeddings = self._process_pointcloud_features(
                    projected_patch_embeddings, pointcloud, pointcloud_projector
                )
