    ...
    *,
    ...
    def restructure(traj):
        ...
        if standardize_fn is not None:
            ...
            # preserve episode metadata if standardize_fn drops it
            if "episode_metadata" in orig_traj and "episode_metadata" not in traj:
                traj["episode_metadata"] = orig_traj["episode_metadata"]
        ...
        handled_old_obs_keys = {
            k for k in old_obs
            if k in (set(filter(None, image_obs_keys.values()))
            | set(filter(None, depth_obs_keys.values()))
            | set(filter(None, state_obs_keys)))
        }
        for k, v in old_obs.items():
            if k in handled_old_obs_keys:
                continue
            if k in new_obs:
                continue
            new_obs[k] = v
