...
from robomimic.models.skill_goal_encoder import GoalVAEEncoder
...
class DiffusionPolicyUNet(PolicyAlgo):
    def _create_networks(self):
        ...
        # Skill-conditioning configuration (e.g., goal-image VAE latent)
        self.skill_encoder = None
        self.skill_latent_dim = 0
        self.skill_goal_key = None
        self.skill_normalize_input = True
        self.skill_trainable = False
        self.skill_image_size = None

        skill_cfg = getattr(self.algo_config, "skill_conditioning", None)
        if skill_cfg is not None and getattr(skill_cfg, "enabled", False):
            if not hasattr(skill_cfg, "goal_key") or skill_cfg.goal_key is None:
                raise ValueError("skill_conditioning.goal_key must be specified when skill conditioning is enabled.")
            if not hasattr(skill_cfg, "vae_checkpoint") or skill_cfg.vae_checkpoint is None:
                raise ValueError("skill_conditioning.vae_checkpoint must be provided when skill conditioning is enabled.")

            self.skill_goal_key = skill_cfg.goal_key
            self.skill_normalize_input = getattr(skill_cfg, "normalize_input", True)
            self.skill_trainable = not getattr(skill_cfg, "freeze", True)
            self.skill_image_size = getattr(skill_cfg, "image_size", 256)
            if isinstance(self.skill_image_size, (list, tuple)):
                if len(self.skill_image_size) == 1:
                    self.skill_image_size = (self.skill_image_size[0], self.skill_image_size[0])
                else:
                    self.skill_image_size = tuple(self.skill_image_size[:2])
            else:
                self.skill_image_size = (int(self.skill_image_size), int(self.skill_image_size))

            self.skill_encoder = GoalVAEEncoder(
                checkpoint_path=skill_cfg.vae_checkpoint,
                latent_dim=getattr(skill_cfg, "latent_dim", None),
                config_path=getattr(skill_cfg, "vae_config_path", None),
                freeze=not self.skill_trainable,
            ).to(self.device)
            self.skill_latent_dim = self.skill_encoder.latent_dim
            if self.skill_trainable:
        ...
        noise_pred_net = ConditionalUnet1D(
            ...
            global_cond_dim=obs_dim*self.algo_config.horizon.observation_horizon + self.skill_latent_dim
        )
        ...
        if self.skill_encoder is not None:
            nets['skill_encoder'] = self.skill_encoder

        nets = nets.float().to(self.device)
        if self.skill_encoder is not None:
            self.skill_encoder = nets['skill_encoder']
    ...
    def _format_goal_images(self, goal_tensor):
        if goal_tensor is None:
            return None
        if not isinstance(goal_tensor, torch.Tensor):
            goal_tensor = torch.as_tensor(goal_tensor, device=self.device)
        goal_tensor = goal_tensor.to(self.device, dtype=torch.float32)

        if goal_tensor.ndim == 5:
            # Assume shape (B, T, C, H, W) and take first temporal index
            goal_tensor = goal_tensor[:, 0]
        elif goal_tensor.ndim == 5 and goal_tensor.shape[-1] in (1, 3):
            goal_tensor = goal_tensor.permute(0, 1, 4, 2, 3)[:, 0]

        if goal_tensor.ndim == 4 and goal_tensor.shape[1] not in (1, 3) and goal_tensor.shape[-1] in (1, 3):
            goal_tensor = goal_tensor.permute(0, 3, 1, 2)
        elif goal_tensor.ndim == 3:
            if goal_tensor.shape[0] in (1, 3):
                goal_tensor = goal_tensor.unsqueeze(0)
            elif goal_tensor.shape[-1] in (1, 3):
                goal_tensor = goal_tensor.permute(2, 0, 1).unsqueeze(0)
            else:
                raise ValueError(f"Unexpected goal image shape: {goal_tensor.shape}")

        if goal_tensor.ndim != 4:
            raise ValueError(f"Goal tensor must have 4 dimensions (B, C, H, W), got {goal_tensor.shape}")

        if goal_tensor.max() > 1.5:
            goal_tensor = goal_tensor / 255.0
        goal_tensor = goal_tensor.clamp(0.0, 1.0)
        if self.skill_image_size is not None:
            goal_tensor = F.interpolate(
                goal_tensor,
                size=self.skill_image_size,
                mode="bilinear",
                align_corners=False,
            )
        if self.skill_normalize_input:
            goal_tensor = goal_tensor * 2.0 - 1.0
            goal_tensor = goal_tensor.clamp(-1.0, 1.0)
        return goal_tensor

    def _encode_skill_latent(self, goal_obs):
        if self.skill_encoder is None or goal_obs is None:
            return None

        keys = self.skill_goal_key
        ...
        latents = []
        for key in key_list:
            if goal_obs is None or key not in goal_obs:
                continue
            goal_tensor = self._format_goal_images(goal_obs[key])
            if goal_tensor is None:
                continue
            latent = self.skill_encoder(goal_tensor)
            if not self.skill_trainable:
        ...
        if len(latents) == 1:
            ...
            latent = torch.stack(latents, dim=0).mean(dim=0)
        return latent
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            ...
            #     if "goal_obs" in batch:
            #         skill_latent = self._encode_skill_latent(batch["goal_obs"])
    ...
    def _get_action_trajectory(self, obs_dict):
        ...
        if isinstance(obs_dict, dict):
            if "goal_obs" in obs_dict:
                goal_inputs = obs_dict["goal_obs"]
            elif self.skill_goal_key is not None:
                keys = self.skill_goal_key if isinstance(self.skill_goal_key, (list, tuple)) else [self.skill_goal_key]
                goal_inputs = {k: obs_dict[k] for k in keys if k in obs_dict}
                if not goal_inputs:
                    goal_inputs = None
