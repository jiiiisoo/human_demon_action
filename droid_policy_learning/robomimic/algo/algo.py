...
class PolicyAlgo(Algo):
    ...
    def compute_batch_visualize(self, batch, num_samples, savedir=None):
        ...
        skill_keys = []
        if hasattr(self, "skill_goal_key") and self.skill_goal_key is not None:
            if isinstance(self.skill_goal_key, (list, tuple)):
                skill_keys = list(self.skill_goal_key)
            else:
                skill_keys = [self.skill_goal_key]

        if goal_obs is not None and skill_keys:
            for key in skill_keys:
                if key in goal_obs:
                    goal_images = goal_obs[key][:num_samples]
                    if isinstance(goal_images, np.ndarray):
                        goal_images = torch.from_numpy(goal_images)
                    if torch.is_tensor(goal_images):
                        if goal_images.dim() == 5:
                            goal_images = goal_images[:, 0]
                        if goal_images.dim() == 4:
                            if goal_images.shape[-1] in (1, 3):
                                pass
                            elif goal_images.shape[1] in (1, 3):
                                goal_images = goal_images.permute(0, 2, 3, 1)
                        images[f"goal_{key}"] = goal_images
