...
from droid_policy_learning.data import (
    ModuleSpec,
...
def train(config, device):
    ...
    if ds_format == "droid_rlds":
        ...
        if config.train.goal_mode is not None:
            goal_kwargs = {}
            if config.train.goal_mode == "offset":
                goal_kwargs["offset"] = config.train.goal_offset
            elif config.train.goal_mode == "last":
                pass
            else:
                raise NotImplementedError(
                    f"Goal mode '{config.train.goal_mode}' is not yet supported in the RLDS pipeline."
                )
            traj_transform_kwargs["goal_relabeling_strategy"] = config.train.goal_mode
            if goal_kwargs:
                traj_transform_kwargs["goal_relabeling_kwargs"] = goal_kwargs
        ...
        dataset_statistics_info = getattr(dataset, "dataset_statistics", None)
        if dataset_statistics_info is None:
            dataset_statistics_info = combined_dataset_statistics

        if dataset_statistics_info is not None:
            if isinstance(dataset_statistics_info, list):
                total_transitions = sum(stat["num_transitions"] for stat in dataset_statistics_info)
            else:
                num_transitions_info = dataset_statistics_info.get("num_transitions", None)
                if isinstance(num_transitions_info, list):
                    total_transitions = sum(num_transitions_info)
                else:
                    total_transitions = num_transitions_info
            if total_transitions is not None:
                print(f"[Data] RLDS dataset approx transitions: {total_transitions}")
        ...
        rlds_dataset_stats = (
            dataset_statistics_info[0]
            if isinstance(dataset_statistics_info, list)
            else dataset_statistics_info
        )
        ...
        print(f"[Data] Training dataset size: {len(train_loader.dataset)} samples")
        if validset is not None:
            print(f"[Data] Validation dataset size: {len(validset)} samples")
    ...
    skill_cfg = getattr(config.algo, "skill_conditioning", None)
    if skill_cfg is not None and getattr(skill_cfg, "enabled", False):
        print("[Skill Conditioning] enabled")
        print(f"  goal_key: {getattr(skill_cfg, 'goal_key', None)}")
        print(f"  latent_dim: {getattr(skill_cfg, 'latent_dim', None)}")
        print(f"  image_size: {getattr(skill_cfg, 'image_size', None)}")
        print(f"  normalize_input: {getattr(skill_cfg, 'normalize_input', True)}")
        print(f"  freeze: {getattr(skill_cfg, 'freeze', True)}")
        print(f"  vae_checkpoint: {getattr(skill_cfg, 'vae_checkpoint', None)}")
    else:
        print("[Skill Conditioning] disabled")
    ...
    validation_enabled = config.experiment.validate and (valid_loader is not None)
    if config.experiment.validate and not validation_enabled:
        print("[Warning] Validation requested but no validation loader available for data_format "
              f"{config.train.data_format}. Skipping validation.")
...
def main(args):
    ...
    if args.debug:
        ...
        # keep debug artifacts contained but still under the user-specified root
        config.train.output_dir = os.path.join(config.train.output_dir, "debug_runs")
