...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        Args:
            ...
            goal_mode (str): one of ["last", "geom", "offset"] or None. Defaults to None (no goal)

            goal_offset (int): number of steps ahead of the start index to treat as the goal when
                goal_mode="offset". Positive values look into the future, negative values look backward.
        ...
        if self.goal_mode is not None:
            assert self.goal_mode in ["last", "geom", "offset"]
            if self.goal_mode == "offset" and self.goal_offset is None:
                raise ValueError("goal_mode 'offset' requires goal_offset to be specified")
    ...
    def get_item(self, index):
        ...
        elif self.goal_mode == "offset":
            goal_index = index_in_demo + self.goal_offset
            if goal_index < 0:
                goal_index = 0
            elif goal_index >= end_index_in_demo:
                goal_index = end_index_in_demo - 1
            offset_steps = goal_index - index_in_demo
            if offset_steps <= self.seq_length - 1:
                action_padding_len = self.seq_length - offset_steps - 1
