...
class LAQTrainer(nn.Module):
            ...
            if self.use_tensorboard and self.tensorboard_writer is not None:
                for key, value in logs.items():
                    if isinstance(value, (int, float)):
                        self.tensorboard_writer.add_scalar(key, value, self.steps)
    ...
    def train(self, log_fn = noop):
        ...
        if self.accelerator.is_main_process:
            run_name = self.wandb_run_name if self.wandb_run_name else self.results_folder_str.split('/')[-1]
            
            # Initialize wandb if enabled
            if self.use_wandb:
                wandb.init(project=self.wandb_project, name=run_name, config={
                    "learning_rate": self.lr,
                    "batch_size": self.batch_size,
                    "num_train_steps": self.num_train_steps,
                })
                print(f"WandB logging enabled - Project: {self.wandb_project}, Run: {run_name}")
            ...
            if self.use_tensorboard:
                ...
                self.tensorboard_writer = SummaryWriter(
                    log_dir=os.path.join(self.tensorboard_log_dir, run_name)
                )
                print(f"TensorBoard logging to: {os.path.join(self.tensorboard_log_dir, run_name)}")
            
            # Warn if no logging is enabled
            if not self.use_wandb and not self.use_tensorboard:
                print("⚠️  Warning: No logging enabled (both wandb and tensorboard are disabled)")
