...
class ImageVideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        try :
            ...
            # Sort by frame number, handling both 'frame0000.jpg' and other formats
            def get_frame_number(filename):
                try:
                    # For 'frame0000.jpg' format
                    if filename.startswith('frame') and filename.endswith('.jpg'):
                        return int(filename[5:-4])  # Extract number from 'frame0000.jpg'
                    # For other formats, try to extract number from filename
